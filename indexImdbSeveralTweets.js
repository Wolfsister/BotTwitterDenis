...
function posterTweet(contenuTweet, idTweetReponse){
	...
	T.post('statuses/update', { status: contenuTweet , in_reply_to_status_id: idTweetReponse  }, function(err, data, response) {
			...
			// console.log("Réponse de tweet envoyé : " + JSON.stringify(response, null, 2)); 
			console.log("Data de tweet envoyé : " + JSON.stringify(data, null, 2)); // contient l'id_str du tweet envoyé 
		  });
		  ...
		  res.on("end",function(){
			...
			*/
			...
			if(jsonFilm.Response == "False"){
				...
				var tweetReponseFilm2 = jsonFilm.Title + " : " + jsonFilm.Plot ;
				var arrayTweetsReponseFilm = [];
				arrayTweetsReponseFilm.push(tweetReponseFilm1);
				arrayTweetsReponseFilm.push(tweetReponseFilm2);

				console.log("Film trouvé : " +tweetReponseFilm1 + tweetReponseFilm2);
				
				//posterTweet(tweetReponseFilm1, idTweetReponse);

				console.log("Longueur du deuxième tweet : " + tweetReponseFilm2.length)
				//posterTweet(tweetReponseFilm2, idTweetReponse);

				postSeveralTweets(arrayTweetsReponseFilm, jsonTweet);
				// TODO : faire fonction postSeveralTweets(arrayTweets)
				sauverTweetRepondu(jsonTweet);
...
//test manuel
/*
T.post('statuses/update', { status: "Reponse manuelle pour test de conversation" , in_reply_to_status_id: "733978819997077504"  }, function(err, data, response) {
			//console.log(data);
			console.log("Tweet Envoyé");
			// console.log("Réponse de tweet envoyé : " + JSON.stringify(response, null, 2)); 
			console.log("Data de tweet envoyé : " + JSON.stringify(data, null, 2)); // contient l'id_str du tweet envoyé 
...
function postSeveralTweets(arrayTextTweetsToPost, jsonTweetToAnswer){

	console.log("dans postSeveralTweets");

	//POST premier tweet, avec récupération de son id_str


	var idTweetToAnswer = jsonTweetToAnswer.id_str; // répond au tweet de l'utilisateur, puis on répondra au tweet créés par le bot

	for(var i in arrayTextTweetsToPost){
		console.log(idTweetToAnswer);
		console.log(arrayTextTweetsToPost[i]);
		idTweetToAnswer = postTweetIdRetrieve(arrayTextTweetsToPost[i], idTweetToAnswer);  //TODO faire fonction postTweetDataRetrieve
	}

}


function postTweetIdRetrieve(text, idTweetToAnswer){ //return id du tweet créé

	T.post('statuses/update', { status: text , in_reply_to_status_id: idTweetToAnswer  }, function(err, data, response) {
